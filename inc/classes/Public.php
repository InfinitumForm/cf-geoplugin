...
if (!class_exists('CFGP_Public', false)) : class CFGP_Public extends CFGP_Global
{
    public function __construct()
    {
        if (CFGP_Options::get('enable_css', 0)) {
            ...
            $this->add_action('wp_ajax_cfgp_display_control_css', [$this, 'get_generated_css']);
            $this->add_action('wp_ajax_nopriv_cfgp_display_control_css', [$this, 'get_generated_css']);
        }
	...
	async function cfgp_display_control_css () {
		...
		if( css ) {
			const response = await fetch('<?php echo esc_url(admin_url('/admin-ajax.php')); ?>', {
				...
				console.error('<?php
                    echo esc_js( __( 'The geo controller could not update the CSS.', 'cf-geoplugin' ) );
        ?>');
...
/* ]]> */
    ...
    public function append_geo_tags()
    {
        ...
        if ($post && is_object($post) && in_array($post->post_type, CFGP_Options::get('enable_geo_tag', []), true)) {
            $geo_data = apply_filters('cfgp/public/geo_tags', [
    ...
    {
        ob_start([&$this, 'output_buffer_callback'], 0, PHP_OUTPUT_HANDLER_REMOVABLE);
    }
    ...
    public function output_buffer_callback($content)
    {
        ...
        if ($API = CFGP_U::api(false, CFGP_Defaults::API_RETURN)) {
            ...
            foreach (apply_filters('cfgp/render/tags', $API) as $key => $value) {
                if (in_array($key, $remove_tags, true)) {
                    continue;
                }
