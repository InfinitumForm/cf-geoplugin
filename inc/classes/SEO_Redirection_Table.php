...
if (!class_exists('CFGP_SEO_Table', false)) : class CFGP_SEO_Table extends WP_List_Table
{
    ...
    {
        if (CFGP_Options::get('enable_seo_csv', 0) && in_array($which, ['top', 'bottom'], true)) {
            global $wpdb;
            $exists = CFGP_U::has_seo_redirection();
            echo '<div class="alignleft actions bulkactions">';

            $seo_import_csv = add_query_arg([
                'action' => 'import',
    ...
    {
        return [
            'cfgp_seo_country'   => ['country', true],
            'cfgp_seo_region'    => ['region', true],
            'cfgp_seo_city'      => ['city', true],
            'cfgp_seo_postcode'  => ['postcode', true],
            'cfgp_seo_http_code' => ['http_code', true],
            'cfgp_seo_only_once' => ['only_once', true],
        ];
    }
                    ...
                    switch ($column_name) {
                        case 'cb':
                            echo '<th scope="row" class="check-column">' . sprintf(
                                '<input type="checkbox" id="cb-select-%1$d" name="seo_redirection[]" value="%1$d" />',
                                esc_attr($rec->ID)
                            ). '</th>';
                            break;
                        case 'cfgp_seo_url':
                        ...
                        case 'cfgp_seo_region':
                            ...
                            if ($term = get_term_by('name', $rec->region, 'cf-geoplugin-region')) {
                                if (!empty($term->description)) {
                                    $region_code = ' (' . esc_html($term->description) . ') ';
                                }
                            }
