    ...
    public static function set($name_or_array = [], $value = null)
    {
        ...
        if (!empty($name_or_array)) {
            if (is_array($name_or_array)) {
                ...
                if (in_array($name, $filter, true) !== false) {
    ...
    public static function delete($name_or_array)
    {
        ...
        if (is_array($name_or_array)) {
            ...
            foreach ($name_or_array as $key) {
                if (isset($options[$key]) && in_array($key, $filter, true) !== false) {
                    unset($options[$key]);
                }
            }
        } elseif (isset($options[$name_or_array]) && in_array($name_or_array, $filter, true) !== false) {
    ...
    public static function sanitize($str)
    {
        if (is_array($str)) {
            ...
            if (is_numeric($str)) {
                ...
                if (intval($str) == $str) {
                    $str = intval($str);
                } elseif (floatval($str) == $str) {
                    $str = floatval($str);
                }
            } elseif (filter_var($str, FILTER_VALIDATE_URL) !== false) {
                return esc_url($str);
            } elseif (preg_match('/^([0-9a-z-_.]+@[0-9a-z-_.]+.[a-z]{2,8})$/i', $str)) {
                $str = trim($str, '&$%#?!.;:,');
                ...
                $str = ($str ? true : false);
            } elseif (!is_bool($str) && in_array(strtolower($str), ['true','false'], true)) {
    ...
    public static function sync_with_the_old_version_of_the_plugin()
    {
        ...
        if ($old_options) {
            ...
            foreach ($old_options as $option => $value) {
                if (in_array($option, CFGP_Defaults::OPTIONS, true)) {
