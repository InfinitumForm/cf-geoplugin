...
class CFGP_Metabox extends CFGP_Global {
	...
	public function __construct(){
		...
		$this->add_action('send_headers', 'send_headers');
	}
		...
		if(!is_admin()){
			...
			foreach($seo_redirection as $data) {
				...
				foreach($country as $c){
					if(in_array($c, $current_country) !== false){
						$redirect[] = 1;
						break;
					}
				}
				...
				foreach($region as $r){
					if(in_array($r, $current_region) !== false){
						$redirect[] = 1;
						break;
					}
				}
				
				if(in_array($current_city, $city) !== false){
					$redirect[] = 1;
				}
				
				if(in_array($current_postcode, $postcode) !== false){
					$redirect[] = 1;
				}
