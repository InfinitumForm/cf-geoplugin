...
if (!class_exists('CFGP_IP')) :
    class CFGP_IP extends CFGP_Global
    {
        ...
        public static function get()
        {
            ...
            $findIP = apply_filters('cfgp/ip/constants', array_merge($findIP, [
                'HTTP_X_REAL_IP',
            ...
            // Let's ask server?
            $external_servers = apply_filters('cfgp/ip/external_servers', [
                'https://ident.me',
        ...
        public static function blocked($list = [])
        {
            ...
            $blacklist = apply_filters('cfgp/ip/blacklist', [
        ...
        public static function server()
        {
            ...
            $findIP = apply_filters('cfgp/ip/server_constants', [
                'SERVER_ADDR',
                'SERVER_NAME',
                'LOCAL_ADDR',
            ]);
            ...
            if (version_compare(PHP_VERSION, '5.3.0', '>=') && function_exists('gethostname')) {
                $gethostname = preg_replace(['~https?:\/\/~','~^w{3}\.~'], '', gethostbyname(gethostname()));

                return CFGP_Cache::set('IP-server', $gethostname);
            } elseif (version_compare(PHP_VERSION, '5.3.0', '<') && function_exists('php_uname')) {
                $gethostbyname = preg_replace(['~https?:\/\/~','~^w{3}\.~'], '', gethostbyname(php_uname('n')));
        ...
        public static function validate_any($ip)
        {
            ...
            if (function_exists('filter_var') && !empty($ip) && filter_var($ip, FILTER_VALIDATE_IP) !== false) {
                return $ip;
            } elseif (!empty($ip) && preg_match('/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/', $ip)) {
                return $ip;
            }
