...
/**
 * CFGEO field validation
 * @since  0.0.4
 */
class cfGeo_Meta_Box_Sanitize {

	/**
	 * A CFGEO field object
	 * @var cfGeo_Meta_Box_field object
	 */
	public $field;
	 ...
	 * @since 1.1.0
	 * @param object $field A CFGEO field object
	...
	public function default_sanitization( $value ) {
		...
		$updated = apply_filters( 'cfgeo_validate_'. $this->field->type(), null, $value, $this->object_id, $this->field->args(), $this );
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = cfGeo_Meta_Box::timezone_string();

		$offset = cfGeo_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new cfGeo_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = cfGeo_Meta_Box::image_id_from_url( $value );
		}
