...
spl_autoload_register('cfGeo_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'cfgeo_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new cfGeo_Meta_Box( $meta_box );
}

define( 'CFGEO_METABOX_URL', cfGeo_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class cfGeo_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'cfgeo_styles' => true, // Include cfgeo bundled stylesheet
		'fields'     => array(),
	);
	...
	/**
	 * Whether CFGEO nonce has been added to the page. (oly add once)
	 * @var   bool
	 * @since 1.2.0
	 */
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'cfgeo_allow_frontend', true, $meta_box );
		...
		$show_filters = 'cfGeo_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'cfgeo_show_on', array( $show_filters, $filter ), 10, 2 );
		}

		// register our scripts and styles for cfgeo
	...
	/**
	 * Registers scripts and styles for CFGEO
	 * @since  1.0.0
	 */
	public function register_scripts() {
		...
		// scripts required for cfgeo
		$scripts = array( 'jquery', 'jquery-ui-core', 'cfgeo-datepicker', /*'media-upload', */'cfgeo-timepicker' );
		// styles required for cfgeo
		$styles = array();
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::CFGEO_VERSION );
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CFGEO_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'knight' ),
					'defaultString' => __( 'Default', 'knight' ),
					'pick'          => __( 'Select Color', 'knight' ),
					'current'       => __( 'Current Color', 'knight' ),
				) );
			}
		} else {
		...
		wp_register_script( 'cfgeo-datepicker', CFGEO_METABOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'cfgeo-timepicker', CFGEO_METABOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cfgeo-scripts', CFGEO_METABOX_URL .'js/cfgeo'. $min .'.js', $scripts, self::CFGEO_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'cfgeo-scripts', 'cfgeo_l10', apply_filters( 'cfgeo_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All', 'knight' ),
		) ) );

		wp_register_style( 'cfgeo-styles', CFGEO_METABOX_URL . 'style'. $min .'.css', $styles );
	...
	/**
	 * Enqueues scripts and styles for CFGEO
	 * @since  1.0.0
	 */
	public function do_scripts( $hook ) {
		// only enqueue our scripts/styles on the proper pages
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			wp_enqueue_script( 'cfgeo-scripts' );

			// default is to show cfgeo styles on post pages
			if ( $this->_meta_box['cfgeo_styles'] )
				wp_enqueue_style( 'cfgeo-styles' );
		}
	}
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'cfgeo_show_on', true, $this->_meta_box ) )
	...
	public function user_metabox() {
		...
		// default is to NOT show cfgeo styles on user profile page
		if ( $this->_meta_box['cfgeo_styles'] != false )
			wp_enqueue_style( 'cfgeo-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		echo "\n<!-- Begin CFGEO Fields -->\n";
		do_action( 'cfgeo_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table cfgeo_metabox">';
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new cfGeo_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
		do_action( 'cfgeo_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CFGEO Fields -->\n";
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new cfGeo_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
					echo '<h2 class="cfgeo-group-name">'. $label .'</h2>';
				if ( $desc )
					echo '<p class="cfgeo_metabox_description">'. $desc .'</p>';
			echo '</th></tr>';
		}
	...
	public static function render_group_row( $field_group, $remove_disabled ) {
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				<table class="cfgeo-nested-table" style="width: 100%;">';
				if ( $field_group->options( 'group_title' ) ) {
					echo '
					<tr class="cfgeo-group-title">
						<th colspan="2">
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new cfGeo_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'cfgeo_show_on', true, $meta_box ) )
			return;
		...
		do_action( "cfgeo_save_{$object_type}_fields", $object_id, $meta_box['id'], self::$updated, $meta_box );
	 ...
	 * @since  1.0.1
	 * @return string URL to CFGEO resources
	 */
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$cfgeo_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $cfgeo_url = str_replace(
		...
		$cfgeo_url = set_url_scheme( $cfgeo_url );

		return trailingslashit( apply_filters('cfgeo_meta_box_url', $cfgeo_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'cfgeo_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "cfgeo_override_option_get_$option_key", 'cfgeo_no_override_option_get', $default );

		if ( $test_get !== 'cfgeo_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'cfgeo_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "cfgeo_override_option_save_$option_key", 'cfgeo_no_override_option_save', $to_save );

		if ( $test_save !== 'cfgeo_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_cfgeo_oembed_handler', array( 'cfGeo_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cfgeo_oembed_handler', array( 'cfGeo_Meta_Box_ajax', 'oembed_handler' ) );

/**
 * A helper function to get an option from a CFGEO options array
 * @since  1.0.1
 * @param  string  $option_key Option key
 * @param  string  $field_id   Option array field key
 * @return array               Options array or specific field
 */
function cfgeo_get_option( $option_key, $field_id = '' ) {
	return cfGeo_Meta_Box::get_option( $option_key, $field_id );
}

/**
 * Get a CFGEO field object.
 * @since  1.1.0
 * @param  array  $field_args  Field arguments
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              cfGeo_Meta_Box_field object
 */
function cfgeo_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	cfGeo_Meta_Box::set_object_id( $object_id );
	cfGeo_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new cfGeo_Meta_Box_field( $field_args );
}
 ...
 */
function cfgeo_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = cfgeo_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function cfgeo_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		cfgeo_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function cfgeo_print_metabox( $meta_box, $object_id ) {
	$cfgeo = new cfGeo_Meta_Box( $meta_box );
	if ( $cfgeo ) {

		cfGeo_Meta_Box::set_object_id( $object_id );

		if ( ! wp_script_is( 'cfgeo-scripts', 'registered' ) )
			$cfgeo->register_scripts();

		wp_enqueue_script( 'cfgeo-scripts' );

		// default is to show cfgeo styles
		if ( $meta_box['cfgeo_styles'] != false )
			wp_enqueue_style( 'cfgeo-styles' );

		cfGeo_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function cfgeo_save_metabox_fields( $meta_box, $object_id ) {
	cfGeo_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 * @return string             CFGEO html form markup
 */
function cfgeo_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = cfGeo_Meta_Box::set_mb_defaults( $meta_box );
	...
	cfGeo_Meta_Box::set_object_type( cfGeo_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		// check nonce
		isset( $_POST['submit-cfgeo'], $_POST['object_id'], $_POST['wp_meta_box_nonce'] )
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], cfGeo_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		cfgeo_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'cfgeo_frontend_form_format', '<form class="cfgeo-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cfgeo" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'knight' ) );
