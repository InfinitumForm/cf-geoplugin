...
?>
<h3><span class="fa fa-certificate"></span> <?php _e('License Control',CFGP_NAME); ?></h3>
...
<form method="post" enctype="multipart/form-data" action="<?php echo $url->url; ?>" target="_self" id="license-form" autocomplete="off">
    ...
    <input type="text" name="license_key" value="<?php echo isset($_POST['license_key']) ? $_POST['license_key'] : (get_option('cf_geo_license_key') ? get_option('cf_geo_license_key') : ''); ?>" placeholder="CF*********************Ge0" autocomplete="off" style="width: 300px;"> 
    <strong><a href="https://cfgeoplugin.com/#price" target="_blank">   <?php CFGP_ACTIVATED ? _e('GET NEW LICENSE KEY',CFGP_NAME) : _e('GET MY LICENSE KEY',CFGP_NAME); ?>   </a></strong><br><br>
    ...
    <?php
        ...
        foreach($options as $key=>$val){
            $active = isset($_POST['sku']) && $_POST['sku'] == $key ? ' selected' : ($key == $sku ? ' selected' : '');
    ...
    <button type="submit" class="button button-primary"><?php CFGP_ACTIVATED ? _e('Expand License',CFGP_NAME) : _e('Activate Unlimited',CFGP_NAME); ?></button>
